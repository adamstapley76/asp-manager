...
function safeResponseBodyForLog(text, apiKey, successPayload) {
  if (successPayload) {
    return JSON.stringify({
      addressResolved: Boolean(successPayload?.postcode || successPayload?.formatted_address)
    });
  }
...
module.exports = async function getAddressAddress(request, response) {
  ...
  if (!apiKey) {
    console.error('Ideal Postcodes address proxy is missing an API key');
  ...
  const upstreamUrl = new URL(`${IDEAL_POSTCODES_COMPATIBILITY_URL}/get/${encodeURIComponent(id)}`);
