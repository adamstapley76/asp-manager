...
Deno.serve(async (request) => {
  ...
  const html = `<!doctype html><html><body style="margin:0;background:#f4f6f8;font-family:Arial,sans-serif;color:#172334"><main style="max-width:600px;margin:24px auto;background:#fff;border-radius:14px;padding:32px"><h1 style="margin:0 0 8px;color:#10263f">Adam Stapley Plumbing</h1><p>Hi ${esc(customer.name || 'there')},</p><p>Your ${kind} <b>${esc(document.document_number)}</b> is ready to view.</p><div style="background:#f6f8fb;border-radius:10px;padding:18px"><b>${esc(document.title)}</b><br><span style="color:#596579">Total: ${esc(money(document.total))}${document.due_date ? ` &middot; ${kind === 'quote' ? 'Valid until' : 'Due'} ${esc(date(document.due_date))}` : ''}</span></div><p style="text-align:center;margin:32px 0"><a href="${esc(viewUrl)}" style="display:inline-block;background:#c9a227;color:#10263f;text-decoration:none;font-size:16px;font-weight:800;letter-spacing:.01em;padding:17px 34px;border-radius:999px;box-shadow:0 4px 10px rgba(201,162,39,.28)">View ${kind}</a></p><p>If you have any questions, simply reply to this email.</p><p style="color:#596579;font-size:13px">Adam Stapley Plumbing<br>07966 858348 &middot; info@adamstapley.co.uk</p></main></body></html>`
